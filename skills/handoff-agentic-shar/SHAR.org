...
# Received this file? Save it alone as SHAR.org in an empty directory and run
# `sh SHAR.org`: it unpacks every block and checks them in the Nix shell it
# pins. Then read the Bootstrap chapter. No shell? Each block opened by
# `#+begin_src LANG :tangle PATH` is a file: write its body to PATH, dropping
# the comma from lines that start with `,*` or `,#+`. An agent with a shell
# does this itself before answering, then says which checks ran.
tangle() {
awk '
